...
(function () {
  const QuickEditor = {
      ...
      const modalContent = `
                <div class="bg-[#151f2e] w-full max-w-[700px] h-[800px] max-h-[90vh] rounded-md shadow-2xl animate-in fade-in zoom-in-95 duration-200 flex flex-col overflow-hidden">
                    ...
                    <div class="relative w-full h-[160px] bg-[#0b1622] flex-shrink-0">
                        ...
                        <button onclick="QuickEditor.close()" class="absolute top-4 right-4 text-white/50 hover:text-white transition-colors z-20">
                    ...
                    <div class="flex-1 flex flex-col min-h-0 px-8 pt-16 pb-6 overflow-hidden">
                        ...
                        <div class="mt-8 pt-6 border-t border-slate-800 flex items-center justify-between flex-shrink-0">
                            <button onclick="QuickEditor.delete()" class="text-red-500/60 hover:text-red-500 text-sm font-bold transition-colors">Delete</button>
                            <div class="flex items-center space-x-6">
                                <button onclick="QuickEditor.toggleFavorite()" id="qe-fav-btn" class="${this.isFavorited ? 'text-red-500' : 'text-slate-400'} hover:scale-110 transition-all">
                                    <i class="fa-solid fa-heart text-xl"></i>
                                </button>
                                <button onclick="QuickEditor.save()" class="bg-blue-500 hover:bg-blue-400 text-white px-8 py-2 rounded text-sm font-bold transition-all shadow-lg shadow-blue-500/20 active:scale-95">Save</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    toggleFavorite: function () {
      ...
      btn.className = `${this.isFavorited ? 'text-red-500' : 'text-slate-400'} hover:scale-110 transition-all`;
    },
  ...
  window.QuickEditor = QuickEditor;
  window.openQuickEditor = QuickEditor.openEditor.bind(QuickEditor);
  QuickEditor.init();
})();
