...
<div id="media-list-wrapper" class="flex flex-col lg:flex-row gap-8 mb-12 animate-in fade-in duration-300">
  <aside class="lg:w-[20%] order-2 lg:order-1">
    <div class="sticky top-24 space-y-6">
      <div class="bg-card rounded-xl p-5 space-y-4">
        <div>
          <label class="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-2">Search List</label>
          <div class="relative">
            <input type="text" placeholder="Filter titles..." bind:value={searchQuery}
            ...
            <i class="fa-solid fa-search absolute left-3 top-2.5 text-slate-600 text-xs"></i>
          </div>
        </div>
        <div>
          <label class="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-2">Sort By</label>
      ...
      <div id="media-categories-sidebar" class="bg-card rounded-xl border border-slate-800 overflow-hidden">
        <div class="p-4">
        ...
        <button onclick={() => filterStatus = 'all'} 
                class="category-btn flex items-center justify-between px-4 py-3 text-sm font-medium {filterStatus === 'all' ? 'text-white bg-slate-800/50 border-l-4 border-accent' : 'text-slate-400 hover:text-white border-l-4 border-transparent'} transition-all w-full">
          <span>All {type.charAt(0).toUpperCase() + type.slice(1)}</span>
          <span class="count text-xs text-slate-500 bg-[#0b1622] px-2 py-0.5 rounded">{listDataEntries.length}</span>
        </button>
        {#each statusGroups as group}
          <button onclick={() => filterStatus = group.id}
                  class="category-btn flex items-center justify-between px-4 py-3 text-sm font-medium {filterStatus === group.id ? 'text-white bg-slate-800/50 border-l-4 border-accent' : 'text-slate-400 hover:text-white border-l-4 border-transparent'} transition-all w-full">
            <span>{group.label}</span>
            <span class="count text-xs text-slate-500">{listDataEntries.filter(i => i.status.toLowerCase() === group.id).length}</span>
          </button>
        {/each}
      </div>
    </div>
  ...
  <main id="media-list-container" class="lg:w-[80%] order-1 lg:order-2 space-y-10 min-h-[400px]">
    {#if isLoading}
      <div class="flex items-center justify-center p-20">
        <i class="fa-solid fa-circle-notch fa-spin text-accent text-2xl"></i>
      </div>
    {:else if visibleGroups.length === 0}
      <div class="p-10 text-center text-[20px] text-slate-500">This list is empty... (≖､≖╬)</div>
    {:else}
      {#each visibleGroups as group}
        <section class="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody>
                {#each group.items as item}
                  ...
                  <tr class="group hover:bg-slate-800/30 transition-colors border-b border-slate-800/50 last:border-0">
                    <td class="p-2 text-center">
                      <img 
                        loading="lazy" 
                        src={HakoImage.getCover(type, item.id, 'small')} 
                        alt={displayTitle}
                        onerror={(e) => (e.target.src = item.image || '/assets/covers/placeholder_medium.jpg')}
                        data-media-id={item.id}
                        onclick={() => window.openQuickEditor(item.id)}
                        onmouseover={() => HakoImage.prefetchBanner(type, item.id)}
                    ...
                    <td class="px-4 py-3">
                      <div 
                        ...
                        onclick={() => window.openQuickEditor(item.id)}>{displayTitle}
                      </div>
